...
while True:
    ...
    if MC== '1':
        ...
        if svtxt == 'B':
            ...
            if svtxtBW== 'READ':
                sg.Popup(svtxtB,title='B')

                AsvtxtB= input('NEX or PREVIOUS? ')
                if AsvtxtB == 'NEXT':
                    sg.Popup(svtxtC,title='C')
        ...
        if svtxt == 'C':
            ...
            if svtxtCW== 'READ':
                ...
                if AsvtxtC == 'NEXT':
                    sg.Popup(svtxtD,title='D')
                elif AsvtxtC == 'PREVIOUS':
                    sg.Popup(svtxtB,title='B')
                else:
                    print('Invalid request.')
        ...
        if svtxt == 'D':
            ...
            if svtxtDW== 'READ':
                ...
                if AsvtxtD == 'NEXT':
                    sg.Popup(svtxtE,title='E')
                elif AsvtxtD == 'PREVIOUS':
                    sg.Popup(svtxtC,title='C')
                else:
                    print('Invalid request.')
    ...
    if MC== '6':
        print('First of all read README.txt file to know what sould you do with this switch.') 
        Par_Dir= input('Where is your Folder destination? (Parent of folder you want to hide)\n')        
        if os.path.exists(Par_Dir)== True:
            Fol_Name= input(r'Now type your folder name: ')
            Fol_Pass  = input('Type your folder password.')
            Fol_Pass_C= input('Re-type your folder password.')
            if Fol_Pass == Fol_Pass_C:
                op_HPbat= open(Par_Dir+'\XF_Hide.Pass.bat', mode='w')
                op_HPbat.write('''@ECHO OFF
if EXIST "Control Panel.{21EC2020-3AEA-1069-A2DD-08002B30309D}" goto UNLOCK
if NOT EXIST '''+ Fol_Name +''' goto MD'''+ Fol_Name +
''':CONFIRM
...
goto CONFIRM
:LOCK
ren'''+ Fol_Name +''' "Control Panel.{21EC2020-3AEA-1069-A2DD-08002B30309D}"
...
set/p "pass=>"
if NOT %pass%== '''+Fol_Pass+'''goto FAIL
attrib -h -s "Control Panel.{21EC2020-3AEA-1069-A2DD-08002B30309D}"
ren "Control Panel.{21EC2020-3AEA-1069-A2DD-08002B30309D}" '''+ Fol_Name +'''
...
:MD'''+ Fol_Name +'''
md '''+ Fol_Name +'''
echo '''+ Fol_Name +'''folder created successfully
goto End
:End''')
                op_HPbat.close()
            else:
                ('Password and it\'s confirmation do not match.')
        else:
            print('Address you\'ve typed is not correct.')
