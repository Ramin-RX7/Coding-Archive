...
# TODO:
    ...
    < Crypto:
        ...
        > Check internet for getting prices
    > Add pause after printing output
    > Edit Decrypted DB (open vscode of dec and wait for user to press "enter" in terminal)
...
def Login(Usernames,Password,no_fingerprint=False):
    ...
    wrong = 0 if not no_fingerprint else 5
    while wrong != 5:
        try:
            rx.cls()
  ...
  while True:
   try:
    ...
    #] IP & WiFi & Mac Address 
        ...
        print()
        pause()
    elif Regex:=re.match(r"sys(tem)?( )?info(rmation)?",Inp,re.IGNORECASE):
        print()
        output = rx.terminal.getoutput("systeminfo")
        #platform.system() == "Windows"
        Patterns = {
                "Device Name": r"Host Name:( )*(?P<result>.*)\n",
                "System Install Date": r"Original Install Date:( )*(?P<result>.*)\n",
                #"Boot Time": r"System Boot Time:( )*(?P<result>.*)\n",
                "System Manufacturer": r"System Manufacturer:( )*(?P<result>.*)\n",
                "System Model": r"System Model:( )*(?P<result>.*)\n",
                "OS": r"OS Name:( )*(?P<result>.*)\n",
            }
        for attr in Patterns:
            if Regex:=re.search(Patterns[attr],output):
                print(f"{attr}: {Regex.group('result')}")
        print()
        pause()
    ...
    #] MTF
    elif Regex:=re.match(r'mtf (?P<name>.+)',Inp,re.IGNORECASE):
        pass
...
class Cryptocurrency:
    ...
    def Get_Dollar_Price():
        if DB["Cryptocurrency"]["Dollar_Price"][0] != Today  and  rx.internet.is_connected():
        ...
        if Section == 'wallet':
            def Wallet_func():
                ...
                if cc_wlt_inp.lower() == 'exit':
                    ...
                    #Exit()
                    if not rx.internet.is_connected():
                        print("[-] An Error Occured (Maybe No Internet Connection.)\n",'red')
                        pause()
                    ...
                    if Name not in Data['Currencies']:
                        ...
                        Data['Currencies'][Name] = {"Name":Name,"Symbol":Symbol,"Transactions":[]}
        ...
        if Section == 'market':
            ...
            if Prices:
             ...
             print(tabulate(Table,tablefmt="fancy_grid",
                ...
                print("[-] An Error Occured (Maybe No Internet Connection.)",'red')
            print()
...
if __name__ == "__main__":
    ...
    try:
        if len(sys.argv) > 1:
            ...
            MainThread = threading.Thread(target=Main,DB=DB)
            MainThread.start()
